...
Route::get('/sftp-test', function () {
    try {
        return Storage::disk('pfizer_ftp_out_folders')->files('/');
    } catch (\Exception $e) {
...
Route::prefix('api')->middleware('auth:sanctum')->withoutMiddleware([VerifyCsrfToken::class])->group(function () {
    ...
    Route::get('/out/orders-sent-to-noName', [CronController::class, 'ordersSentToPfizer'])->name('cron.ordersSentToPfizer');
    Route::get('/out/new-customer-or-change', [CronController::class, 'newCustomerOrChange'])->name('cron.newCustomerOrChange');
    //Route::get('/out/orders-exported-to-noName', [CronController::class, 'ordersExportedToPfizer'])->name('cron.ordersExportedToPfizer');
