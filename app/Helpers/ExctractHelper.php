        ...
        foreach ($lines as $lineKey => $data) {
                ...
                if ($line[0] == '1') {
                    ...
                    if (
                        ...
                        $pharmacy->pharmacy_sent_to_pfizer = 0;
                    } else {
                    ...
                    */
                    ...
                    foreach ($pharmacies_old as $pharmacy_temp) {
                        $pharmacy_temp->pharmacy_sent_to_pfizer = 1;
                        ...
                        foreach ($lastOrders as $orderA) {
                            ...
                            if (isset($order->pharmacy->pharmacy_email) && $order->pharmacy->pharmacy_email != '') {
                                Mail::mailer('smtp')->to($order->pharmacy->pharmacy_email)
                                    //->bcc(['52dda429.NoName.onmicrosoft.com@amer.teams.ms','PfizerDigitalFrance@noName.com'])
...
*/
            ...
            */
            ...
            $res = PfizerHelper::processProductDiscountThresholdFromTradePolicy($offre, $line, $sheet, $spreadsheet, $key, 'N', 'L', 'M', $product->id, 1);
            ...
            $res = PfizerHelper::processProductDiscountThresholdFromTradePolicy($offre, $line, $sheet, $spreadsheet, $key, 'Q', 'O', 'P', $product->id, 2);
            if( isset($res['prods']['update']['premium'][$product->id]) ) {
                foreach($res['prods']['update']['premium'] as $key_2 => $product_juju){
                    $productsUpdated['prods']['update']['premium'][$key_2] = $key_2;
                }
            ...
            $res = PfizerHelper::processProductDiscountThresholdFromTradePolicy($offre, $line, $sheet, $spreadsheet, $key, 'T', 'R', 'S', $product->id, 3);
            if( isset($res['prods']['update']['premium'][$product->id]) ) {
                foreach($res['prods']['update']['premium'] as $key_2 => $product_juju){
                    $productsUpdated['prods']['update']['premium'][$key_2] = $key_2;
                }
    ...
    public static function extractAndProccesPriceControlInCsv ($contents){
        ...
        foreach ($lines as $lineKey => $line_csv) {
            ...
            if ( isset($line[$lineKey][1]) && $line[$lineKey][1] != "" && $line[$lineKey][0] != 'TARIF' ) {
                ...
                if ( isset($product[0]->id) ) {
                    PfizerHelper::processProductDiscountThresholdFromPriceControl($line[$lineKey], $product);
