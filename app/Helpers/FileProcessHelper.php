...
class FileProcessHelper {
    ...
    public static function processCustomerSanitationIn($contents, $file) {
        ...
        $contents = Storage::disk('pfizer_ftp_in_folders')->get($filePath);

        $tempFileName = 'customer_sanitation_' . time() . '.xlsx';
        Storage::disk('pfizer_temp_folder')->put($tempFileName, $contents);
        $filePath = Storage::disk('pfizer_temp_folder')->path($tempFileName);
...
*/
    ...
    public static function processBlockedOrdersIn($contents, $file) {
        ...
        $contents = Storage::disk('pfizer_ftp_in_folders')->get($file);
        ...
        Storage::disk('pfizer_temp_folder')->delete($filePath);
    ...
    public static function processOrdersSentToPfizerOut($folder) {
        PfizerHelper::UsersActions();
        ...
        $orders_cagedim = OrderCagedim::where('order_sent_to_pfizer', 0)
        ...
        $orders_call_center = Order::where('order_sent_to_pfizer', 0)
            ...
            ->select(
                ...
                'order_sent_to_pfizer',
                DB::raw('NULL as order_sent_to_pfizer_date'),
                'orders.created_at',
                'orders.order_source'
            )
            ->get();
        ...
        foreach ($orders as $order) {
            if ( isset($order->order_source) && $order->order_source == 'CALL' || $order->order_source == 'Call' || $order->order_source == 'Apm' || $order->order_source == 'APM') {
                ...
                $order->order_sent_to_pfizer = 1;
                $order->order_sent_to_pfizer_date = Carbon::now();
                ...
                if(isset($pharmacy) && $pharmacy->pharmacy_sap_id != '') {
                    ...
                    $tempOrder->order_sent_to_pfizer = 1;
                    $tempOrder->order_block_code = '';
                    $tempOrder->order_sent_to_pfizer_date = Carbon::now();
                    $tempOrder->save();
                }
                /* else {
        ...
        $fileName = PfizerHelper::insertCurrentDateBeforeExtensionOrder("Order/ORDER_FROM_CMC_TO_PET_.xml");
        ...
        $file_status->file_status_type = "ordersSentToPfizer";
        $file_status->save();

        // Al guardar el XML, eliminamos la declaración XML
        $xmlContent = $dom->saveXML($dom->documentElement);
        Storage::disk('pfizer_ftp_out_folders')->put($fileName, $xmlContent);
        Storage::disk('pfizer_temp_folder')->put($fileName, $xmlContent);
    ...
    public static function processNewCustomerOrChangeOut($folder) {
        ...
        $pharmacies_2 = Pharmacy::where('pharmacy_sent_to_pfizer', '=', 0)
        ...
        foreach($pharmacies_2 as $pharmacy) {
            ...
            $file_save = str_replace('./', '', PfizerHelper::insertCurrentDateBeforeExtension($fileName) );
            ...
            $pharmacy->pharmacy_sent_to_pfizer = 1;
            $pharmacy->save();
        ...
        $pharmacies = Pharmacy::where('pharmacy_sent_to_pfizer', '=', 0)
        ...
        foreach ($pharmacies as $pharmacy) {
            ...
            $historicChanges = PharmacyHistoric::where('pharmacy_historic_pharmacy_id', $pharmacy->id)
                ->where('pharmacy_historic_sent_to_pfizer', 0)
                ->get();
            ...
            if ($historicChanges->isNotEmpty()) {
                ...
                foreach ($historicChanges as $change) {
                    ...
                    $change->pharmacy_historic_sent_to_pfizer = 1;
                    $change->save();
                }
...
/*
    public static function processOrdersExportedToPfizerOut($folder) {
        PfizerHelper::UsersActions();
    }
*/
    public static function processRollingOrderHistoryOut($folder) {
        ...
        $filename_two_week = str_replace('./', '', PfizerHelper::insertCurrentDateBeforeExtension($filename) );
        $file_save = str_replace('./', '', PfizerHelper::insertCurrentDateBeforeExtension($filename) );
        ...
        foreach ($orders as $order) {
            $usefulData[] = [
                ...
                Carbon::parse($order->order_sent_to_pfizer_date)->format(app('global_format_date_hour_minute_export'))
            ];
        }
        ...
        foreach ($orders as $order) {
            if (isset($order->pharmacy_cip)){
                ...
                if ($order->order_block_code == 'Cancelled') {
                    ...
                    $order_status = $order->order_sent_to_pfizer == 1 ? 'Exported' : 'Pending to export';
                }
    ...
    public static function processWeeklyOrderConfirmationsOut($folder) {
        ...
        $orders = Order::where('orders.order_sent_to_pfizer_date', '>=', Carbon::now()->subWeeks(1)->startOfWeek()->format(app('global_format_date') . ' 00:00:00'))
        ...
        $orders = OrderCagedim::where('order_sent_to_pfizer_date', '>=', Carbon::now()->subWeeks(1)->startOfWeek()->format(app('global_format_date') . ' 00:00:00'))
                        ->where('order_sent_to_pfizer', '=', 1)
                ->get();
    ...
    public static function processMonthlyActivityReportingOut($folder) {
        ...
        $orders = Order::where('orders.order_sent_to_pfizer_date', '>=', Carbon::now()->startOfMonth()->subMonths(1)->startOfMonth()->format(app('global_format_date') . ' 00:00:00'))
                        ->where('orders.order_status', Order::EXPORTED)
                ->get();
        ...
        $orders = OrderCagedim::where('order_sent_to_pfizer_date', '>=', Carbon::now()->startOfMonth()->subMonths(1)->format(app('global_format_date') . ' 00:00:00'))
                        ->where('order_sent_to_pfizer', '=', 1)
                ->get();
    ...
    public static function processQuarterlyActivityReportingOut($folder) {
        ...
        $orders = Order::where('orders.order_sent_to_pfizer_date', '>=', Carbon::now()->startOfMonth()->subMonths(3)->format(app('global_format_date') . ' 00:00:00'))
                        ->where('orders.order_status', Order::EXPORTED)
                ->get();
        ...
        $orders = OrderCagedim::where('order_sent_to_pfizer_date', '>=', Carbon::now()->startOfMonth()->subMonths(3)->format(app('global_format_date') . ' 00:00:00'))
                        ->where('order_sent_to_pfizer', '=', 1)
                ->get();
    ...
    public static function processBlockedOrdersOut($folder) {
        ...
        $filename_daily = str_replace('./', '', PfizerHelper::insertCurrentDateBeforeExtension($filename) );
        $file_save = str_replace('./', '', PfizerHelper::insertCurrentDateBeforeExtension($filename) );
 ...
 */
        ...
        $orders = Order::whereBetween('orders.order_sent_to_pfizer_date', [$data_from, $data_to])
        ...
        foreach ($orders as $order) {
            ...
            $usefulData[] = [
                ...
                'NoName',
                $order->pharmacy->pharmacy_zipcode,
                $order->order_sent_to_pfizer_date
            ];
        }

        $orders = OrderCagedim::whereBetween('order_sent_to_pfizer_date', [$data_from, $data_to])
                ->where('order_block_code', '=', '')
                ->whereNotNull('order_sent_to_pfizer_date')
                ->where('order_sent_to_pfizer', '=', 1)
                ->get();
        ...
        $orders = Order::where('order_sent_to_pfizer', 0)
        ...
        // Guardar el contenido en el disco pfizer_ftp_out_folders
        //Storage::disk('pfizer_ftp_out_folders')->put($fileName, $fileContent);
...
/*
    ...
    public static function processProductAudit($folder) {
        ...
        $changes = ProductHistoric::where('product_historic_sent_to_pfizer', 0)
        ...
        foreach ($changes as $change) {
            $change->product_historic_sent_to_pfizer = 1;
            $change->save();
        }
