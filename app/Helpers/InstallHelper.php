...
class InstallHelper {
    ...
    public static function chnageTimeStampsType (){
        ...
        DB::statement('ALTER TABLE orders ALTER COLUMN order_sent_to_pfizer_date DATETIME2 NULL;');
        ...
        DB::statement('ALTER TABLE orders_cagedim ALTER COLUMN order_sent_to_pfizer_date DATETIME2 NULL;');
    ...
    public static function createInitialFolders() {
        ...
        Storage::disk('pfizer_ftp_in_folders')->makeDirectory('in');
        Storage::disk('pfizer_ftp_out_folders')->makeDirectory('out');
        Storage::disk('pfizer_temp_folder')->makeDirectory('temp');
        ...
        foreach ($inFolders as $folder) {
            Storage::disk('pfizer_ftp_in_folders')->makeDirectory('in/' . $folder);
            Storage::disk('pfizer_ftp_in_folders')->makeDirectory('in/' . $folder . '/processed');
            Storage::disk('pfizer_ftp_in_folders')->makeDirectory('in/' . $folder . '/error');
        }

        // Output folders
        $outFolders = [
            'productIntegrationCheck', 'ordersSentToPfizer', 'newCustomerOrChange',
        ...
        foreach ($outFolders as $folder) {
            Storage::disk('pfizer_ftp_out_folders')->makeDirectory('out/' . $folder);
        }
    }
}
