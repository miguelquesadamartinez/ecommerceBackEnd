...
class PfizerHelper {
    public static function getInFiles ($folder){
        ...
        if ($folder == "cagedimOrders") {
            $files = Storage::disk('pfizer_ftp_in_folders')->files('PharmaML');
        } else if ($folder == "pharmacies") {
            $files = Storage::disk('pfizer_ftp_in_folders')->files('Customer_Master');
        } else if ($folder == "cancelledOrders") {
            $files = Storage::disk('pfizer_ftp_in_folders')->files('order_cancellation');
        } else {
            $files = Storage::disk('pfizer_ftp_in_folders')->files($folder);
        }
        ...
        foreach ($files as $file) {
            ...
            $file_save = str_replace('./', '', PfizerHelper::insertCurrentDateBeforeExtension($file) );

            Storage::disk('pfizer_ftp_in_folders')->copy($org_file, 'in/' . $folder."/processed/".$file_save);
            Log::info('Pharmacy test :');
            // ToDo: Take care
            //Storage::disk('pfizer_ftp_in_folders')->delete($org_file);
    ...
    public static function doOutFiles ($folder){
        ...
        } else if ($folder == "ordersSentToPfizer") {
            $return = FileProcessHelper::processOrdersSentToPfizerOut($folder);
        } else if ($folder == "newCustomerOrChange") {
            $return = FileProcessHelper::processNewCustomerOrChangeOut($folder);
//        } else if ($folder == "ordersExportedToPfizer") {
//            $return = FileProcessHelper::processOrdersExportedToPfizerOut($folder);
    ...
    public static function ldapSync (){
        ...
        foreach ($ldapUsers as $ldapUser) {
            ...
            if ($user){
                ...
                $tempPassword = PfizerHelper::generateSecurePassword(12);
    ...
    public static function processProductDiscountThresholdFromTradePolicy($offre, $line, $sheet, $spreadsheet, $key, $priceColumn, $thresholdColumn, $discountColumn, $productId, $levelDelete) {
        ...
        if (isset($line[$priceColumn]) && $line[$priceColumn] != '') {
            ...
            if (strpos($line[$thresholdColumn], '≥') !== false) {
                ...
                $temp_val = PfizerHelper::extractNumbers($line[$thresholdColumn]);
            }
...
*/
    ...
    public static function bindBuilderQuery($sqlArr){
        ...
        foreach($sqlArr as $key => $value) {
            ...
            //print_r($sqlTxt);
            //if (env('LOG_DEBUG'))
                PfizerHelper::DebuggerTxT("$num - $sqlTxt");
        }
    }
