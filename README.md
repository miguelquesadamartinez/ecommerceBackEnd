...
El proyecto incluye una configuración completa de Docker para ejecutar el entorno de desarrollo con todas las dependencias necesarias. La configuración actual está optimizada para desarrollo local con conexión a SQL Server en el host de Windows.

### Arquitectura Docker

```
┌─────────────────────────────────────┐
│  Container: ecommerce_app           │
│  ┌───────────────────────────────┐  │
│  │  PHP 8.2 + Apache             │  │
│  │  - Laravel 12                 │  │
│  │  - SQL Server Drivers         │  │
│  │  - LDAP Support               │  │
│  │  - SSH2/SFTP Extensions       │  │
│  └───────────────────────────────┘  │
│         ↓ Puerto 8000               │
└─────────────────────────────────────┘
              ↓
...
- **Extensiones PHP Instaladas**:
    ...
    - `ldap`: Autenticación LDAP/Active Directory
    - `ssh2`: Conexión SFTP para integración con sistemas NoName
    - `gd`, `zip`, `intl`, `soap`, `bcmath`, `mbstring`: Extensiones estándar Laravel
- **Microsoft ODBC 17**: Driver nativo para comunicación con SQL Server
- **Composer 2.x**: Gestión de dependencias
- **Configuración Apache**: DocumentRoot apuntando a `/public` con rewrite habilitado

### Requisitos Previos

- **Docker Desktop** instalado ([Descargar Docker Desktop](https://www.docker.com/products/docker-desktop))
- **Docker Compose** (incluido con Docker Desktop)
- **SQL Server** instalado localmente en la máquina host Windows
    - SQL Server debe estar configurado para aceptar conexiones TCP/IP
    - Puerto 1433 debe estar abierto en el firewall
    - Autenticación mixta (SQL Server + Windows) habilitada
...
3. **Configurar conexión a SQL Server en .env**:

    Asegúrate de que tu archivo `.env` tenga la configuración correcta para SQL Server:
    ...
    **⚠️ Importante**: **NO uses `localhost` o `127.0.0.1`** dentro del contenedor Docker, ya que estos apuntan al contenedor mismo, no a tu máquina Windows. Usa siempre `host.docker.internal`.
...
5. **Instalar dependencias de Composer** (si no se instalaron automáticamente):
...
El contenedor Docker se conectará a la instancia de SQL Server instalada en tu máquina Windows. Asegúrate de que:

1. **SQL Server esté aceptando conexiones TCP/IP**:
    - Abre **SQL Server Configuration Manager**
    - Ve a **SQL Server Network Configuration** → **Protocols for [TU_INSTANCIA]**
    - Habilita **TCP/IP** (clic derecho → Enable)
    - Reinicia el servicio de SQL Server

2. **El puerto 1433 esté habilitado en el firewall**:
...
3. **Usa `host.docker.internal` en DB_HOST**: Este hostname especial de Docker resuelve a la IP de tu máquina Windows host desde dentro del contenedor

4. **La autenticación de SQL Server esté configurada correctamente** (Mixed Mode si usas usuario `sa`)

### Solución de Problemas Comunes

**Problema**: El contenedor no se conecta a SQL Server

- **Solución 1**: Verifica que `DB_HOST=host.docker.internal` está en tu `.env`
- **Solución 2**: Asegúrate de que SQL Server escuche en `0.0.0.0` (todas las interfaces), no solo `127.0.0.1`
- **Solución 3**: Ejecuta `docker-compose down` y luego `docker-compose up -d` después de cambiar el `.env`

**Problema**: "No services to build" warning

- **Es normal**: Significa que la imagen ya está construida. Los contenedores deberían iniciarse igualmente.
...
# Probar comando específico de importación
...
**Nota**: En producción Windows, las tareas se ejecutan mediante `schedule.bat` con Task Scheduler. En Docker, debes ejecutar `php artisan schedule:run` manualmente o configurar un cron dentro del contenedor.
...
DB_CONNECTION=sqlsrv
DB_HOST=host.docker.internal  # ⚠️ NO usar localhost
DB_PORT=1433
DB_DATABASE=NonameEcommerce
DB_USERNAME=sa
...
# Configuración de almacenamiento SFTP (en Docker usa local)
...
│   └── app/
│       └── private/
│           └── noName/        # Archivos SFTP (mapeado al contenedor)
│               ├── in/        # Archivos entrantes de NoName
│               └── out/       # Archivos salientes a NoName
└── ...
```

### Diferencias Docker vs Producción Windows

| Aspecto            | Docker (Desarrollo)   | Windows IIS (Producción)      |
| ------------------ | --------------------- | ----------------------------- |
...
├── Dockerfile                  # Imagen personalizada de PHP 8.2 con Apache
├── .dockerignore              # Archivos excluidos del contexto de Docker
├── .env.docker                # Plantilla de configuración para Docker
└── docker/
    └── php/
        └── local.ini          # Configuración personalizada de PHP
```
...
- Verifica que SQL Server esté ejecutándose en tu máquina local
- Asegúrate de que `DB_HOST=host.docker.internal` está en tu archivo `.env`
- Verifica que el puerto 1433 esté abierto en el firewall
...
Si necesitas trabajar con el frontend (assets JavaScript/CSS), puedes ejecutar Vite **fuera del contenedor** en tu máquina host:

```bash
# En tu máquina Windows (no dentro de Docker)
npm install
npm run dev  # Vite en modo desarrollo con hot reload
```

El servidor Vite correrá en tu host y el contenedor Docker accederá a los assets a través del volume compartido.

### Notas de Rendimiento

- **Volúmenes**: Los archivos están mapeados directamente desde Windows al contenedor. Esto puede afectar el rendimiento en Windows debido a cómo Docker Desktop maneja los volúmenes.
- **Composer Install**: Se ejecuta durante la construcción de la imagen para optimizar el tiempo de inicio.
- **Cache de Laravel**: Los directorios `storage/` y `bootstrap/cache/` tienen permisos optimizados para escritura.

### Desarrollo en Producción (Alternativa sin Docker)

Si prefieres desarrollar sin Docker (como en producción), puedes usar los scripts incluidos:
...
Estos scripts utilizan `concurrently` para ejecutar múltiples procesos en paralelo.
...
- **Formato**: Excel (.xlsx)
- **Generación**: `FileProcessHelper::processOrdersSentToNomaneOut()`
